...
app.innerHTML = ` <div id="page-auth"> <div class="auth-card"> <div class="logo">Kulturo</div> <p class="tagline">Votre journal culturel personnel</p> <div class="auth-tabs"> <button class="auth-tab active" id="tab-login"  onclick="UI.switchAuthTab('login')">Connexion</button> <button class="auth-tab"        id="tab-signup" onclick="UI.switchAuthTab('signup')">Inscription</button> </div> <div class="auth-form" id="auth-form"> <div class="form-group"> <label>Email</label> <input type="email" id="auth-email" placeholder="vous@exemple.com" /> </div> <div class="form-group"> <label>Mot de passe</label> <input type="password" id="auth-password" placeholder="••••••••" /> </div> <button class="btn btn-primary" style="width:100%" onclick="UI.handleAuth()">Se connecter</button> </div> <div class="auth-divider">ou</div> <button class="auth-demo-btn" onclick="UI.enterDemo()">✨ Essayer en mode démo (sans compte)</button> </div> </div>`;
}
...
grid.innerHTML = Array(8).fill(0).map(() => ` <div class="skeleton-card"> <div class="skeleton skeleton-cover"></div> <div class="skeleton-body"> <div class="skeleton skeleton-line"></div> <div class="skeleton skeleton-line short"></div> <div class="skeleton skeleton-line xshort"></div> </div> </div>`).join(””);
}
try {
...
return `<button class="filter-chip ${State.filters.status === s ? "active" : ""}" onclick="UI.setStatusChip('${s}')">${label}</button>`;
}).join(””);
...
grid.innerHTML = `<div class="empty-state"> <div class="empty-icon">🎭</div> <h3>Rien ici</h3> <p>${emptyMsg}</p> ${f.search || f.favorite || f.status !== "all" || f.type !== "all" ?`<button class="btn btn-secondary" onclick="UI.navTo('library')">Voir tout</button>` : emptyBtn} </div>`;
return;
}
...
return ` <article class="${classes}" style="animation-delay:${Math.min(i*25,250)}ms" onclick="UI.openEditModal('${e.id}')"> ${coverHTML} ${starsHTML(e.rating, e.is_favorite)} </article>`;
}
...
const profileTopHTML = !State.demoMode ? ` <div class="profile-section profile-identity-bar"> <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap"> <div style="display:flex;align-items:center;gap:.5rem;flex:1;min-width:200px"> <span style="font-size:1.5rem">👤</span> <div> <div style="font-size:.75rem;color:var(--text-3)">Connecté en tant que</div> <div style="font-size:.88rem;font-weight:600;color:var(--text-1)">${esc(State.user?.email||"")}</div> </div> </div> <div style="display:flex;align-items:center;gap:.5rem"> <input type="text" id="input-username" placeholder="Ton pseudo…" maxlength="30" value="${esc(cachedUsername)}" style="font-size:.85rem;padding:.35rem .65rem;background:var(--bg-3);border:1px solid var(--border);border-radius:var(--radius);color:var(--text-1);width:140px" /> <button class="btn btn-primary btn-sm" onclick="UI.saveUsername()">Enregistrer</button> </div> <button class="btn btn-ghost btn-sm" onclick="UI.signOut()">Déconnexion</button> </div> </div>` : “”;
...
const monthBars = monthCounts.map((n, i) => `<div class="month-col"> <div class="month-bar-wrap"> <div class="month-bar" style="height:${Math.round(n/maxMonth*100)}%" title="${n} ajout${n>1?"s":""}"></div> </div> <div class="month-label">${MONTHS[i]}</div> ${n ?`<div class="month-count">${n}</div>` : ""} </div>`).join(””);
...
? topYear.map((e,i) => `<div class="top-row" onclick="UI.openEditModal('${e.id}')"> <span class="top-rank">${i+1}</span> ${e.cover_url ?`<img src="${esc(e.cover_url)}" class="top-cover" alt="" loading="lazy">`:`<div class="top-cover top-cover-placeholder">${TYPE_ICONS[e.media_type]}</div>`} <span class="top-title">${esc(e.title)}</span> <span class="top-rating">${ratingStars(e.rating)}</span> </div>`).join(””)
: `<p style="color:var(--text-3);font-size:.85rem">Aucun média noté en ${_profileYear}.</p>`;

const barHTML = (label, value, tot, color) => ` <div class="bar-item"> <div class="bar-item-label"><span>${label}</span><span>${value}</span></div> <div class="bar-track"><div class="bar-fill" style="width:${Math.round(value/tot*100)}%;background:${color}"></div></div> </div>`;
...
return ` <div class="rating-hist-col" title="${n} média${n !== 1 ? "s" : ""} · ${note}/10"> <div class="rating-hist-count">${n || ""}</div> <div class="rating-hist-bar${isPeak ? " peak" : ""}" style="height:${px}px"></div> </div>`;
}).join(””);

const ratingsHTML = totalRated > 0 ? `<div class="profile-section"> <div class="rating-hist-header"> <h3 class="profile-section-title" style="margin:0">Notes</h3> <div class="rating-hist-meta"> <span class="rating-hist-total">${totalRated} notes</span> ${avgRating ?`<span class="rating-hist-avg">moyenne <strong>${avgRating}</strong>/10</span>` : ""} </div> </div> <div class="rating-hist">${ratingBars}</div> <div class="rating-hist-legend"> <span>1★</span> <span>5★★★★★</span> </div> </div>` : “”;
...
root.innerHTML = `<div class="modal-overlay" id="modal-overlay" onclick="UI.closeModalOnBg(event)"> <div class="modal" role="dialog" aria-modal="true"> <div class="modal-header"> <h3>${isEdit ? "Modifier" : "Ajouter un média"}</h3> <button class="btn-icon" onclick="UI.closeModal()">${iconX()}</button> </div> <div class="modal-body"> <!-- Recherche API --> <div class="form-group"> <label>Catégorie *</label> <select id="f-type" onchange="UI.onTypeChange()"> <option value="game"  ${entry?.media_type==="game" ?"selected":""}>🎮 Jeu vidéo</option> <option value="movie" ${entry?.media_type==="movie"?"selected":""}>🎬 Film / Série</option> <option value="book"  ${entry?.media_type==="book" ?"selected":""}>📚 Livre</option> </select> </div> <div class="form-group" id="api-search-group"> <label>Recherche automatique <span style="color:var(--text-3);font-weight:400" id="api-avail-label"></span></label> <div class="api-search-wrap"> <input type="text" id="f-api-search" placeholder="Chercher un titre…" autocomplete="off" /> <div class="api-results" id="api-results" style="display:none"></div> </div> </div> <!-- Champs --> <div class="form-group"> <label>Titre *</label> <input type="text" id="f-title" value="${esc(entry?.title||"")}" placeholder="Titre du média" /> </div> <div class="form-row"> <div class="form-group"> <label>Statut</label> <select id="f-status"> ${["wishlist","playing","finished","paused","dropped"].map(s =>`<option value=”${s}” ${(entry?.status===s||((!entry)&&s===“finished”))?“selected”:””}>${STATUS_LABELS[s]}</option>`
).join(””)}
</select>
</div>
...
const fillPct = filledFull ? “100%” : filledHalf ? “50%” : “0%”;
return ` <span class="star-wrap" style="position:relative;display:inline-block;width:28px;height:28px;cursor:pointer"> <svg viewBox="0 0 20 20" width="28" height="28" style="display:block"> <defs> <linearGradient id="sg${i}" x1="0" x2="1" y1="0" y2="0"> <stop offset="${fillPct}" stop-color="var(--accent)"/> <stop offset="${fillPct}" stop-color="var(--border-2)"/> </linearGradient> </defs> <polygon points="10,2 12.9,7.6 19,8.5 14.5,12.9 15.6,19 10,16 4.4,19 5.5,12.9 1,8.5 7.1,7.6" fill="url(#sg${i})"/> </svg> <button type="button" style="position:absolute;left:0;top:0;width:50%;height:100%;background:none;border:none;cursor:pointer;padding:0" onclick="UI.setRating(${half})" onmouseenter="UI.previewRating(${half})" onmouseleave="UI.clearPreview()" title="${half}/10"></button> <button type="button" style="position:absolute;right:0;top:0;width:50%;height:100%;background:none;border:none;cursor:pointer;padding:0" onclick="UI.setRating(${full})" onmouseenter="UI.previewRating(${full})" onmouseleave="UI.clearPreview()" title="${full}/10"></button> </span>`;
}).join(””);
...
results.innerHTML = items.map((it, idx) => `<div class="api-result-item" onclick="UI.fillFromApi(${idx})"> ${it.cover_url ?`<img class="api-result-thumb" src="${it.cover_url}" alt="" loading="lazy">`:`<div class="api-result-thumb" style="display:flex;align-items:center;justify-content:center;font-size:1.5rem">${TYPE_ICONS[type]}</div>`} <div class="api-result-info"> <div class="api-result-title">${esc(it.title)}</div> <div class="api-result-sub">${it.release_year||""} ${it.author||""}</div> </div> </div>`).join(””);
...
root.insertAdjacentHTML(“beforeend”, ` <div class="modal-overlay confirm-overlay" id="confirm-overlay" style="z-index:1100;background:rgba(0,0,0,.6)"> <div class="modal confirm-modal" style="max-width:360px" role="alertdialog" aria-modal="true"> <div class="modal-header"><h3>${esc(title)}</h3></div> <div class="modal-body" style="padding-top:.5rem"> <p style="color:var(--text-2);font-size:.9rem">${esc(message)}</p> </div> <div class="modal-footer"> <button class="btn btn-secondary" id="confirm-cancel">Annuler</button> <button class="btn btn-${variant}" id="confirm-ok">${esc(confirmLabel)}</button> </div> </div> </div>`);
...
return `<article class="media-card discover-card" data-discover-idx="${idx}"> ${cover} <div class="card-body"> <div class="card-title">${esc(it.title)}</div> <div class="card-meta"> <span class="badge badge-${it.media_type}">${TYPE_ICONS[it.media_type]} ${TYPE_LABELS[it.media_type]}</span> ${it.release_year ?`<span style="font-size:.72rem;color:var(--text-3)">${it.release_year}</span>`: ""} </div> ${it.author ?`<div style="font-size:.75rem;color:var(--text-3);margin-top:.2rem">${esc(it.author)}</div>`: ""} ${it.groq_reason ?`<div class="discover-reason">✦ ${esc(it.groq_reason)}</div>`: it.description ?`<div style="font-size:.75rem;color:var(--text-2);margin-top:.35rem;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">${esc(it.description)}</div>` : ""} <div style="display:flex;gap:.4rem;margin-top:.75rem"> <button class="btn btn-secondary btn-sm" style="flex:1" onclick="UI.addToWishlist(${idx})">+ Wishlist</button> <button class="btn btn-ghost btn-sm" title="Pas intéressé" onclick="UI.ignoreDiscover(${idx})">✕</button> </div> </div> </article>`;
}
...
const root = document.getElementById(“modal-root”);
root.innerHTML = `<div class="modal-overlay" id="modal-overlay" onclick="UI.closeModalOnBg(event)"> <div class="modal detail-modal" role="dialog" aria-modal="true"> <div class="modal-header"> <div style="display:flex;align-items:center;gap:.5rem"> <span class="badge badge-${e.media_type}">${TYPE_ICONS[e.media_type]} ${getTypeLabel(e)}</span> <span class="badge badge-${e.status}">${STATUS_LABELS[e.status]}</span> ${e.is_favorite ?`<span style="color:var(--accent);font-size:1rem">♥</span>`: ""} </div> <div style="display:flex;align-items:center;gap:.4rem;margin-left:auto"> ${externalHTML}${youtubeHTML} <button class="btn-icon" onclick="UI.closeModal()">${iconX()}</button> </div> </div> <div class="detail-body"> <div class="detail-cover">${cover}</div> <div class="detail-info"> <h2 class="detail-title">${esc(e.title)}</h2> <div class="detail-stars">${stars}</div> <div class="detail-meta"> ${metaRow("Genre", e.genre)} ${metaRow("Auteur", e.author)} ${metaRow("Plateforme", e.platform)} ${metaRow("Année", e.release_year)} ${metaRow("Ajouté le", e.created_at ? new Date(e.created_at).toLocaleDateString("fr-FR") : null)} </div> ${synopsisHTML} ${e.notes ?`<div class="detail-notes"><div class="detail-notes-label">Notes personnelles</div><p>${esc(e.notes)}</p></div>` : ""} </div> </div> <div class="modal-footer"> <button class="btn btn-danger btn-sm" onclick="UI.deleteEntry('${e.id}')">Supprimer</button> <button class="btn btn-secondary" onclick="UI.closeModal()">Fermer</button> <button class="btn btn-primary" onclick="UI.openEditFromDetail('${e.id}')">✏ Modifier</button> </div> </div> </div>`;
}
...
el.style.cssText = `left: ${Math.random()*100}vw; background: ${colors[Math.floor(Math.random()*colors.length)]}; width: ${4 + Math.random()*6}px; height: ${8 + Math.random()*8}px; animation-delay: ${Math.random()*600}ms; animation-duration: ${900 + Math.random()*800}ms; transform: rotate(${Math.random()*360}deg); border-radius: ${Math.random()>0.5?"50%":"2px"};`;
container.appendChild(el);
...
html += localMatches.map(e => `<div class="quick-result" onclick="UI.openEditModal('${e.id}')"> ${e.cover_url ?`<img src="${esc(e.cover_url)}" class="quick-thumb" alt="">`:`<div class="quick-thumb quick-thumb-ph">${TYPE_ICONS[e.media_type]||“🎭”}</div>`} <div class="quick-info"> <div class="quick-title">${esc(e.title)}</div> <div class="quick-sub">${TYPE_LABELS[e.media_type]} · ${STATUS_LABELS[e.status]}</div> </div> </div>`).join(””);
}
html += `<div class="quick-section-label">Ajouter depuis les APIs <span id="quick-api-spinner" class="quick-spinner"></span></div> <div id="quick-api-results"></div>`;
qa.innerHTML = html;
...
return Object.entries(groups).map(([date, items]) => `<div class="activity-date-group"> <div class="activity-date-label">${date}</div> ${items.map(e => activityRowHTML(e)).join("")} </div>`).join(””);
}
...
return `<div class="activity-row"> ${coverHTML} <div class="activity-info"> <div class="activity-line"> <span class="activity-username">${esc(e.username)}${meLabel}</span> <span class="activity-verb">${status}</span> </div> <div class="activity-title">${icon} ${esc(e.title)}</div> <div class="activity-meta"> <span class="badge badge-${e.media_type}" style="font-size:.7rem">${type}</span> ${starsHTML} ${e.is_favorite ?`<span style="color:var(--accent)">♥</span>` : ""} </div> </div> <div class="activity-time">${new Date(e.created_at).toLocaleTimeString("fr-FR", { hour:"2-digit", minute:"2-digit" })}</div> </div>`;
}
